...
def analyze_priority(description: str, service_name: str = "") -> tuple[str, str, int]:
    """Given description and service name, return (priority, reasoning, confidence)."""
    system_prompt = (
        "You are a civic operations AI triage agent. Analyze the civic issue description and "
        "respond ONLY with strict JSON: {\"priority\": \"low\"|\"medium\"|\"high\"|\"critical\", \"reasoning\": \"one sentence\", \"confidence\": <int 0-100>}"
    )
    ...
    if any(k in combined_text for k in CRITICAL_KEYWORDS):
        return "critical", "Rule Agent: Immediate safety/hazard keywords detected in complaint.", 75
    if any(k in combined_text for k in HIGH_KEYWORDS):
        return "high", "Rule Agent: High-impact infrastructure issue detected.", 75
    if any(k in combined_text for k in LOW_KEYWORDS):
        return "low", "Rule Agent: Standard non-urgent maintenance request.", 70
    if any(k in combined_text for k in MEDIUM_KEYWORDS):
        return "medium", "Rule Agent: Regular operational issue detected.", 70

    return "medium", "Rule Agent: Default priority assigned based on standard SLA.", 65
